...
async def get_practice_sequence(target_symbol: str = "Ψ"):
    """获取练习阶段序列"""
    try:
...
async def get_test_sequence(target_symbol: str = "Ψ"):
    """获取测试阶段序列"""
    try:
...
@router.post("/sessions", response_model=AttentionSessionResponse, status_code=status.HTTP_201_CREATED)
...
async def update_session_route(
    test_session_id: int, update_data: AttentionSessionUpdate, session: Session = Depends(get_session)
):
    """更新测试会话信息"""
...
async def create_record_route(record_data: AttentionRecordCreate, session: Session = Depends(get_session)):
    """保存单个注意力测试记录"""
    try:
...
async def save_row_records_route(
    ...
    """保存一行符号的所有记录"""
    try:
        records = save_row_records(session, test_session_id, user_id, row_data)
