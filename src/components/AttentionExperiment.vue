      ...
      <div class="content-area">
        ...
        <div v-else-if="phase === 'practice'" class="symbol-container">
          ...
          <div class="symbol-grid practice-grid">
            <div 
              v-for="(symbol, index) in currentRow" 
              :key="index" 
              class="symbol-cell"
              :class="{ 'clicked': symbol.is_clicked }"
...
.compact-grid-view {
  ...
  border-radius: 4px;
  box-shadow: 0 2px 12px 0 rgba(0, 0, 0, 0.1);
  min-height: calc(100vh - 160px); /* 将高度调整为几乎整个视口高度 */
  overflow: hidden;
  margin-bottom: 10px; /* 减小下方留白 */
}
...
.compact-grid {
  ...
  display: flex;
  flex-direction: column;
  justify-content: space-between; /* 确保行均匀分布 */
}
...
.grid-row {
  ...
  justify-content: space-between; /* 确保列均匀分布 */
}
